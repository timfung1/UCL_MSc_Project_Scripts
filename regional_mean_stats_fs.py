...
#FREESURFER7 regions
#get regional mean stats from an image (e.g. NODDI or tau PET)
...
parser.add_argument('in_file', type=str,
                    help='Metric image to extract from (e.g. NODDI/PET)')
parser.add_argument('roi_file',type=str,
                    help='ROI parcellation file')
parser.add_argument('--out',type=str,
                    help='Optional output file for NODDI values')
args=parser.parse_args()

# Load the input image
in_img = nib.load(args.in_file)
vox_dim = np.product(in_img.get_header().get_zooms())

in_image=in_img.get_data().ravel()
valid_point = np.isfinite(in_image)

# load the parcelation
roi_image = nib.load(args.roi_file).get_data().ravel().astype('int')
...
for k in sorted(rois.iterkeys()):
    ...
    for label in v:
        ...
        im_val = in_image[roi_image[valid_point]==label].mean()
        im_sum += im_val*volume
        vol_sum += volume
    val=im_sum/vol_sum
    val_dict[k]=val
    print('{}: {:.6f}'.format(k,val))
final_output=""
for k in sorted(val_dict.iterkeys()):
    final_output += '{:.6f}, '.format(val_dict[k])
print(','.join(sorted(val_dict.keys())))
print(final_output)
if args.out:
