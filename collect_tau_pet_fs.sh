...
# 10th June 2024
# CALLS PYTHON SCRIPT
# COLLECT ALL TAU PET STATS USED FOR FAD (sampled from pet space with resampled freesurfer labels)
...
echo "COLLECTING TAU PET FROM PET SPACE"

OUT_FILE=${data_root}/output/stats/FAD_tau-pet_fs_stats_${date_stamp}.csv
echo $HEADER > $OUT_FILE

for PET in `ls ${data_root}/output/tau_pet_suvr/FAD*/ses-*/pet/FAD-*_tau-suvr_ref-inf-cereb-gm.nii.gz` ; do
    sub=`echo $PET | cut -d / -f 7`
    ses=`echo $PET | cut -d / -f 8`
    echo $sub, $ses

    SCRATCH_OUT=`mktemp taustats.XXXXXX`
    LAB=${data_root}/output/labels_pet_space/${sub}/${ses}/anat/${sub}_${ses}_fs-aparc-aseg_space-pet_res.nii.gz
    echo ${sub} ${ses}
    python $stats_script ${PET} ${LAB} --out ${SCRATCH_OUT}
    echo "${sub},${ses},tau_pet,"`cat ${SCRATCH_OUT}` >> ${OUT_FILE}
